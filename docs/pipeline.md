...
7. **Candidatos semânticos:** `ClipAnalysisProvider` local retorna artefato JSON versionado com janelas, hook, título publicável, razão, categoria, completude e dependência de contexto. O backend envia título e canal do vídeo como contexto. O modo local combina a frase mais forte com uma pessoa ou entidade comprovada pelo título/transcrição; com Ollama, a identificação do protagonista faz parte da análise estruturada. Todo título é fiel, não enganoso e limitado a 100 caracteres para publicação no YouTube.
